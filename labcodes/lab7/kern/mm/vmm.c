...
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
     ...
     *    page_insert ï¼ build the map of phy addr of an Page with the linear addr la
     *    swap_map_swappable ï¼ set the page swappable
     */
         if(swap_init_ok) {
             struct Page *page=NULL;
             // (1ï¼According to the mm AND addr, try to load the content of right disk page into the memory
             //     which page managed.
             swap_in(mm, addr, &page);
             // (2) According to the mm, addr AND page, setup the map of phy addr <---> logical addr
             page_insert(mm->pgdir, page, addr, perm);
             swap_map_swappable(mm, addr, page, 1); // (3) make the page swappable.
             page->pra_vaddr = addr; // set page's vaddr
         }
         else {
