...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2012011894 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	 ...
	 *       NOTICE: useful function: sfs_bmap_load_nolock, sfs_buf_op
	*/
    blkoff = offset % SFS_BLKSIZE;  // offset inside the first block
    if (blkoff != 0) { // (1)
        size = (nblks != 0) ? (SFS_BLKSIZE - blkoff) : (endpos - offset); // how much to Rd/Wr in first block
        // begin Rd/Wr
        ...
        alen += size;   // operated size increase
        if (nblks == 0) {  // no more to Rd/Wr
            goto out;
        }
        buf += size;    // forward buffer
        blkno++;        // next block no.
        nblks--;        // remain blocks
    }

    // (2)
    size = SFS_BLKSIZE; // for aligned blocks, always Rd/Wr the whole block
    while (nblks > 0) { // still have aligned blocks to Rd/Wr
        // begin Rd/Wr
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) {
            goto out;
        }
    ...
    // (3)
    blkoff = 0;                  // begin from block start
    size = endpos % SFS_BLKSIZE; // how much to Rd/Wr in the last block
    if (size > 0) {
        // begin Rd/Wr
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) {
            goto out;
